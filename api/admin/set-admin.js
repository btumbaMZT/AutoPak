...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!makeAdmin) {
      // Never let the app end up with zero admins — there'd be no way to
      // promote anyone back without direct database access.
    ...
    const rows = await sql`update users set is_admin = ${makeAdmin} where id = ${userId} returning id`;
